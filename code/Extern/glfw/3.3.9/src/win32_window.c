                                   ...
                                   WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        ...
        //            {
        //                PTOUCHINPUT input = &pInputs[i];
        //                const uint64_t fingerId = (input->dwID + 1);
        //                const int x = (input->x / 100.f) - pos.x;
        //                const int y = (input->y / 100.f) - pos.y;
        //                _glfwInputTouch(window, fingerId, x, y, input->dwFlags & TOUCHEVENTF_UP);
        //            }
        //            bHandled = TRUE;
        ...
        case WM_POINTERUP:
        {
            const int pointerId = GET_POINTERID_WPARAM(wParam);
            ...
            if (GetPointerType(pointerId, &type) && type == PT_TOUCH)
            {
                ...
                if (GetPointerTouchInfo(pointerId, &touchInfo))
                {
                    POINT pos = { 0, 0 };
                    ClientToScreen(hWnd, &pos);
                    const int x = GET_X_LPARAM(lParam) - pos.x;
                    const int y = GET_Y_LPARAM(lParam) - pos.y;
                    _glfwInputTouch(window, touchInfo.pointerInfo.pointerId, x, y, uMsg == WM_POINTERUP);
                    return 0;
                }
            }
            break;
...
{
    ULONG flags;
    return IsTouchWindow(window->win32.handle, &flags);
}
#endif
