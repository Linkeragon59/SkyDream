...
void NeatPoleBalancingModule::OnInitialize()
{
	...
	myBalancingGenome = new Neat::Genome("Executables/Neat/PoleBalancing");
	myResettingGenome = new Neat::Genome("Executables/Neat/PoleResetting");
...
void TrainNeat(bool aBalancingTraining)
{
	...
	callbacks.myOnTrainGenerationEnd = [aBalancingTraining, &population, &generationIdx]() {
		...
		if (const Neat::Genome* bestGenome = population.GetBestGenome())
		{
			...
			{
				std::string fileName = aBalancingTraining ? "Executables/Neat/PoleBalancing" : "Executables/Neat/PoleResetting";
